...
namespace StateMachineVisualizer.States
{
    ...
    {
        public SecondState(TimelineStateMachine stateMachine) : base(stateMachine) { }

        public override void OnProcess()
        {
            if (Input.GetKeyDown(KeyCode.Space))
            {
                stateMachine.NextState(new ThirdState(timelineStateMachine));
            }
            
            if (Input.GetKeyDown(KeyCode.E))
            {
                string eventName = RandomStringGenerator.GenerateRandomString(5);
                timelineStateMachine.Controller.AddEvent<BlueEventItem>(eventName, trackName);
            }
